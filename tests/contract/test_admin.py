"""Contract tests for the /admin UI and config-reload endpoints."""
...
def test_admin_generate_token_uses_ui_compatible_format(admin_settings):
    client = _admin_client(admin_settings)
    with client:
        first = client.post("/admin/generate-token")
        second = client.post("/admin/generate-token")

    assert first.status_code == 200
    assert second.status_code == 200
    first_token = first.json()["token"]
    second_token = second.json()["token"]
    assert re.fullmatch(r"tk-[0-9a-f]{48}", first_token)
