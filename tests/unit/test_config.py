"""Unit tests for the JSON config loader and interface/source API."""
...
def test_config_examples_load():
    root = Path(__file__).resolve().parents[2]
    minimal = load_settings(root / "config.json.example")
    windows_local = load_settings(
        root / "config" / "examples" / "config.windows-local.json"
    )

    assert minimal.api_interfaces[0].public_ids() == ["claude"]
    assert any(
        target.name == "z-image-turbo-comfyui"
        for target in windows_local.comfyui_targets
    )
