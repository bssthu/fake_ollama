...
COMFYUI_TARGET_ITEM_SCHEMA: List[Dict[str, Any]] = [
    ...
    {"key": "preset", "type": "string", "default": "z_image_turbo",
     "description": "Declarative workflow preset：z_image_turbo、qwen_image_edit_aio、sensenova_u1、joyai_echo；提供自定义 workflow path + bindings 时使用 custom/comfyui_api。"},
...
MODEL_PROFILE_ITEM_SCHEMA: List[Dict[str, Any]] = [
    ...
    {"key": "capabilities", "type": "string_list",
     ...
     "description": "模型对外声明的 capability 标签，用于 /api/show、Anthropic 模型能力映射和 /playground/api/models 功能发现，不会自动给模型增加能力。/api/show 只报告 Ollama 标准的 completion/tools/vision，媒体标签只在 Playground 专用发现接口中返回。常用值：completion=文本/聊天补全，tools=支持 tool calling，vision=聊天接口可接收图片输入，image_generation=/v1/images/generations 图片生成，image_edit=/v1/images/edits 图片编辑，video_generation=/v1/videos/generations 视频或图生视频。聊天模型通常至少含 completion；纯媒体 workflow 不需要 completion"},
